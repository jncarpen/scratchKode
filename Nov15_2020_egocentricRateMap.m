...
% get position data
position = pos_cm{1,SelectSession};
t = position(:,1);
x = position(:,2); y = position(:,3);
head_direction = get_hd(position);

% get object positions
ObjectPosition = hwCoord{1,SelectSession};
...
% get spike times
spiketimes = SpikeTimes{1,SelectSession}{1,SelectCell};

% get spiketrain
[spikeTrain, spikeTrain_smooth] = binSpikes(t, spiketimes);

% create meshgrid
[t,r] = meshgrid(linspace(0,2*pi,round(360/AngleBin)+1),linspace(0,50,SpaceBin+1));
...
for i=1:1:SpaceBin
    for j=1:1:round(360/AngleBin)
        for k=1:length(position)
            if RToObject(k)>=r(i,j) && RToObject(k)<r(i+1,j) && FacingToObjectAngle(k)>=t(i,j)&&FacingToObjectAngle(k)<t(i,j+1)
                ECOMap(i,j)=ECOMap(i,j)+1;
                if ~isnan(position(k))
                    ECOFiringMap(i,j)=ECOFiringMap(i,j) + spikeTrain(k);
                else
                end
             else
            end
...
for i=1:1:SpaceBin
    for j=1:1:round(360/AngleBin)
        if ECOMap(i,j)>0
            ECOFiringMap_normalized(i,j)=ECOFiringMap(i,j)./ECOMap(i,j);
        else
        end
    end
end
...
% occupancy
ax1=subplot(1,3,1); 
polarplot3d(ECOMap_smoothed,'PlotType','surfn','RadialRange',[0 50]);         
% colormap(ax1,'jet');
daspect([1 1 1]); 
view(2)
camroll(90)
grid off
title('Ecocentric Occupancy','FontName', 'Calibri light', 'FontSize', 14, 'FontWeight', 'normal');

% egocentric firing map
ax2=subplot(1,3,2);
polarplot3d(ECOFiringMap_smoothed,'PlotType','surfn','RadialRange',[0 50]);
% colormap(ax2,'jet');    
daspect([1 1 1]); 
view(2)
camroll(90)
grid off
title('Ecocentric Spike Map','FontName', 'Calibri light', 'FontSize', 14, 'FontWeight', 'normal');

% egocentric firing map (normalized)
ax3=subplot(1,3,3); 
polarplot3d(ECOFiringMap_normalized_smoothed,'PlotType','surfn','RadialRange',[0 50]);         
% colormap(ax3, 'jet');
daspect([1 1 1]);
view(2)
camroll(90)
grid off
title('Ecocentric Spike Map (Normalized)','FontName', 'Calibri light', 'FontSize', 14, 'FontWeight', 'normal');

sgtitle(['Session ',num2str(SelectSession),', Unit ',num2str(SelectCell)], 'FontName', 'Calibri light', 'FontSize', 20, 'FontWeight', 'normal');
