function egoModulation_dMan(input, varargin)
    %% get stuff from dMan
    % parse inputs
    S = dManDe.Settings();
    inp = inputParser();
    ...
    if nanmax(tracker.hd) < 350
        head_direction = rad2deg(tracker.hd);
    else
        head_direction = tracker.hd;
    end
...
ax = gca; % current axes
box(ax, "off")
title(ax, "test title", 'FontName', 'Calibri light', 'FontSize', 14, 'FontWeight', 'normal')

hold on;

% plot theta 1 
h1 = quiver(ax, data(:,1), data(:,2), uprime, vprime, 0); % 0 turns autoscaling off
custom_color = [.3 .5 1];
set(h1, 'Color', custom_color, 'AutoScale', 'off', 'LineWidth',1)
set(ax, 'Visible', 'off')

% plot reference location
refPnt_plot = plot(ax, xGoal, yGoal, 'o', 'MarkerSize', 10);
...
% % plot a second quiver (can delete this later)
% theta_dos = offset;
