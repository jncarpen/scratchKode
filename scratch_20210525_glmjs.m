...
load('/gpfs/home/dwt244/jnc/data/jsdata/dsetPreprocess.mat')
savepath = '/gpfs/home/dwt244/jnc/output/GLM/glm_ego100.mat';
warning('off', 'all');

% load files
load('/gpfs/home/dwt244/jnc/data/Simulation-POP2/ego100.mat');

for iter = 1:86
    disp(['JS Data: running session ', num2str(iter), ' of 86...'])
    
    % data for current unit
    P = dset(iter);
    
    for u = 1:length(dset(iter).unit)
        ST = dset(iter).unit(u);
        out = ego100(iter).out;
        ...
        glm_place100(iter).glmout = glmout;

        if mod(iter,10) == 0
            disp(['saving iteration ', num2str(iter), ' ...']);
            save(savepath, 'glm_ego100', '-v7.3');
        end
    end
