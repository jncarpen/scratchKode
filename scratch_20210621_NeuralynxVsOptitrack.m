%% Compare NeuraLynx with OptiTrack data
% June 21, 2021
% For the JS dataset (animals 26019 and 26191)
% filepaths correspond to external hard-drive locations
% we are interested in position data only here**

% specify animal 
animal = 26019;
recFolderPath = strcat('D:\Data\', num2str(animal));
...
tracking2D = cell(1,length(dirList));
tracking3D = cell(1,length(dirList));

for session= 1:length(dirList)
    % store dirList information in tracking directory
    tracking(session).dir = dirList{session};
    
    folderPath = char(dirList(session));
    
    % grab files with specific extensions
    fileName_csv = dir(fullfile(folderPath, '*.csv'));
    fileList2D = dir(fullfile(folderPath, '*.pos'));
    ...
    if ~isempty(fileName_csv)
        ...
        tracking3D{session} = strcat(fileName_csv.folder, '\', fileName_csv.name);
    else
        tracking3D{session} = [];
    end
    
     for i=1:length(fileList2D)
        filelist2D{i} = append(fileList2D(i).folder, '\', fileList2D(i).name); 
     end
    
 tracking2D{session} = filelist2D;
end

%% now make it into a cell array
totalSessions = length(tracking3D);
optiTrack = cell(1,totalSessions);

for session = 1:totalSessions
    csv_path = tracking3D{1,session};
    
    if ~isempty(csv_path)
        optiTrack{session} = readtable(csv_path, 'HeaderLines',7); 
    else
        optiTrack{session} = [];
    end
end

%% make a 'tracking' struct for each session
clear optiData
...
for session = 1:length(dirList)
    tracking2DNow = tracking2D{session}{:};
    pullPos = io.axona.getPos(tracking2DNow); % from BNT* /interpolate NaNs?
    tracking(session).track2d = pullPos;
end
